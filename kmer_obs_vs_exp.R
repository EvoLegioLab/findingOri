...
slidingwindowplot <- function(windowsize, step_len, inputseq, accession, oligo)
{
  ...
  kmers_compl <- chartr("acgt", "tgca", kmers_found) #To get number of kmers on the complementary strand
  ...
  chunk <- inputseq[1:windowsize-1] #Look at only the first chunk first
  ...
  for (j in 1:length(chunk_comp8)){ 
    ...
    if (names(chunk_comp8[j])==kmers_found[k]) {
      ...
      kmer_matrix[k,1] <- chunk_comp8[j] - expected
  ...
  #freq_mean <- floor(mean(kmer_matrix[,1]))
  #kmer_matrix[,1] = kmer_matrix[,1]-freq_mean
  for (i in 2:n) { #Look at all the remaining chunks
    ...
    chunk_comp1 <- count(chunk,1)
    #freq_mean=0
    for (j in 1:tot) { #For all selected kmers, get the cumulated frequency in each window
      jj <- index_vec[j]
      expected <- get_expected(names(chunk_comp8[jj]), chunk_comp1, oct_per_wind)
      kmer_matrix[j,i] <- kmer_matrix[j,i-1] + chunk_comp8[jj] - expected
      kmer_not_cum[j,i] <- chunk_comp8[jj] - expected
      #kmer_not_cum[j,i] <- expected
      #freq_mean = freq_mean + chunk_comp8[jj]
    }
    #freq_mean = floor(freq_mean/tot)
    #kmer_matrix[,i] <- kmer_matrix[,i] - freq_mean
  ...
  #scaled_gc <- 100*cum_gc_vec
  #min_y = min(c(min(kmer_matrix), min(scaled_gc)))
  #max_y = max(c(max(kmer_matrix), max(scaled_gc)))
  #plot(window_mid,scaled_gc,type="l", col="blue", main = title,  xlab="Middle pos of each window",ylab="B subtilis: Red=Cum freq per 8mer, Blue=Cum GC-skew", ylim=(c(min_y,max_y)))
  #for (m in 1:tot) {
  #  lines(window_mid, kmer_matrix[m,], type = "l", col = "red")
  #}
  scaled_gc <- 50*cum_gc_vec
  min_y = min(c(min(kmer_not_cum), min(scaled_gc)))
  max_y = max(c(max(kmer_not_cum), max(scaled_gc)))
...
step <- window*0.1
slidingwindowplot(window, step, dna_seq, fasta_name, oligo)
